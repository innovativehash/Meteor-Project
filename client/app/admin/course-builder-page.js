...
let tbo         = {};
  ...
  tbo.pages   = [];
    ...
    tbo.name            = "Bobo Dobo";
    tbo.credits         = 3;
    tbo.passing_percent = 60;
    tbo.keywords        = ['this','that'];
...
//-------------------------------------------------------------------
  ...
  'click #cb-next-btn'( e, t ) {
    ...
    if ( tbo.videos.length ) {
      tbo.pages[ Template.instance().page.get() ] = tbo.videos[0];
      tbo.videos  = [];
    ...
    let p   = Template.instance().page.get();
